...
$(document).ready(function() {
	...
	var col = [{field: 'state',align: 'center',valign : 'middle',checkbox: true,visible: true},
	  			   {field: 'metentid',title: '序号',formatter: function (value, row, index) {  
	   				...
	   				{field: 'riskPeriod',title: '险种类别',align: 'center',sortable: true,valign : 'middle'},
	...
	var sortOrder = "metentid asc";
	 ...
	 $("#search").click(function() {
		...
		data.riskPeriod = $("#riskPeriod").val(),
		data.StartDate = $("#StartDate").val(),
		data.endDate = $("#endDate").val();
        date.riskCode="";
        var url_d = "/LmriskAController/selectByParamAll.do";
...
/*		//删除
			...
			if (r==true)
			  {
				...
				$.ajax({
					type : "POST",
					url:path+"/LmriskAController/delete.do",// 后台请求URL地址
					data : "insuranceCom="+ row.insuranceCom+"&riskCode="+row.riskCode,
					dataType : "json",
		...
		'click .update': function (e, value, row, index) {
			...
			var url = "select2.jsp?insuranceCom=" +row.insuranceCom+"&riskCode="+row.riskCode;
	        toedit(url);
...
function actionFormatter(value, row, index) {      
	return [
		'<button class="btn btn-primary update">明细查看</button>'
	].join('');
}
