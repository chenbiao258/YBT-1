...
$(document).ready(function() {
	...
	var init_url = "/menuController/queryByGrpMenuone.do";
	
  var url_d ="/menuController/queryByGrpMenuone.do";
...
function actionFormatter(value, row, index) {      
	return [
		'<button class="btn btn-primary update">二级菜单</button>'+
		'<button class="btn btn-primary remove">三级菜单</button>'
	].join('');
}
...
$(function(){
	...
	function mycheckbox(seq){
		 ...
		 for(var i=0;i<arr.length;i++){
			 insertLDUserTOMenuGrpA(arr[i].submenu,seq,i,arr.length);
		 }
	...
	function insertLDUserTOMenuGrpA(nodecode,menugrpcode,i,j){

		$.ajax({
			type : "POST",
			url:path+"/ldmenugrptomenuaController/saveAddone.do",// 后台请求URL地址
			data :  "menugrpcode="+menugrpcode+"&nodecode="+nodecode,
			dataType :"json",
			success : function(data) {
